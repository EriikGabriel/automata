...
export function StageTransition({
	...
	const currentStep = simulateResult?.steps[simulateStep];
	const sourceLabel = nodes.find((n) => n.id === source)?.data.label;
	const targetLabel = nodes.find((n) => n.id === target)?.data.label;
	const edgeSymbols = labelStr.split(",").map((s) => s.trim());
	const isActive =
		!!currentStep &&
		!!sourceLabel &&
		!!targetLabel &&
		currentStep.currentStates.includes(sourceLabel) &&
		currentStep.nextStates.includes(targetLabel) &&
		edgeSymbols.includes(currentStep.symbol);
	...
	const handleContextMenuOpenChange = useCallback(
		...
		[id, selected, setEdges, setContextMenuOpen],
	);
								...
								<span
									className={cn(
										...
										selected
											? "text-amethyst-500"
											: isActive
												? "text-amethyst-700 animate-pulse"
												: "text-slate-600",
									)}
								>
									{label}
