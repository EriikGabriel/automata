...
export function StateNode({ id, data, selected }: NodeProps<StateNodeType>) {
	...
	const isLastStep = simulateStep === (simulateResult?.steps.length ?? 0) - 1;
	const isActive =
		currentStep?.currentStates.includes(label) ||
		(isLastStep && currentStep?.nextStates.includes(label)) ||
		false;
							...
							{isEditing ? (
								<input
									...
									className={cn(
										...
										selected || isTransitionSource || isActive
